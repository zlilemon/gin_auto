...
func (r *Repository) GetOrderStatusCheck(c *gin.Context, req BillingStatusCheckReq) (billingOrderStatusCheckRespList []*BillingStatusCheckResp, err error) {
	...
	result = database.StoreDB.Debug().Table("order_detail").Where(
		"order_type='bookConsume' and end_unix_time=? and device_status!='CLOSED'", req.CheckUnixTime).Find(&billingRespList)
	//log.Infof("storeDB : %+v", database.StoreDB)

	//result = database.StoreDB.Debug().Table("order_detail").Find(&billingRespList)
